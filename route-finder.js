...
function handleFindRoute() {
    ...
    const selectedStand = standsData.find(stand => stand.id === parseInt(selectedStandId));
    ...
    // Search routes
    const matchedRoutes = searchRoutes(selectedStand, destinationTerm.toLowerCase());
...
function displayResults(matchedRoutes, standName, destinationTerm) {
    ...
    resultsHeader.textContent = `Found ${matchedRoutes.length} Route${matchedRoutes.length > 1 ? 's' : ''}`;
    ...
    matchedRoutes.forEach(route => {
        ...
        distanceItem.innerHTML = route.calculatedDistance !== null
        ...
        const fareValue = route.fare % 1 === 0 ? Math.floor(route.fare) : route.fare;
        fareItem.innerHTML = `₹${fareValue}`;

        // Time
        const timeItem = document.createElement('span');
        timeItem.className = 'route-detail-item';
        timeItem.innerHTML = `🕒 ${route.travel_time}`;
...
function addToRecentSearches(standId, standName, destination) {
    ...
    const existingIndex = recentSearches.findIndex(search =>
        search.standId === standId && search.destination.toLowerCase() === destination.toLowerCase()
    );
